...
function cb_new_mail_from($old) {
	// $cb_mail_sender_email_id = get_option('cb_mail_sender_email_id') 
	if (!empty(get_option('cb_mail_sender_email_id'))){
		return get_option('cb_mail_sender_email_id');
	}
	else{
		trigger_error( "CB mail sender: The *sender email id* has not been set on ".get_bloginfo('url').".", E_USER_NOTICE );
		return($old);
	}
}
function cb_new_mail_from_name($old) {
	if (!empty(get_option('cb_mail_sender_id'))){
		return get_option('cb_mail_sender_id');
	}
	else{
		trigger_error ( "CB mail sender: The *sender id* has not been set on ".get_bloginfo('url').".", E_USER_NOTICE );
		return($old);
	}
}
